...
import { sudoActivarEnlace, sudoDesactivarEnlace, sudoEliminarEnlace } from '@/services/Enlaces';

export default function EnlacesTable({ data }) {
    ...
    function mapData() {
      return data.map((item) => ({
        ...
        dataRow: [
          ...
          { text: item.url },
          { text: item.clicks },
          { text: item.tipoRecordatorio, isRounded: true },
          {
            isRoundedList: true,
            text: Array.isArray(item.tags) ? item.tags.map(tag => tag.name) : [],
          },
  ...
  const headerTable = [
    ...
    { text: "URL", maxW: "220px" },
    { text: "Veces visitado", maxW: "90px" },
    { text: "Tipo de notificación", maxW: "100px" },
    { text: "Tag(s)", maxW: "150px" },
    { text: "Estado", maxW: "70px" },
  ]

  const handleDeleteItem = async (itemId)=>{
    if(!itemId)return

    const isEliminated = await sudoEliminarEnlace(itemId)
  ...
  const handleDisableItem= async (itemId) =>{
    ...
    const isDisabled = await sudoDesactivarEnlace(itemId)
  ...
  const handleEnableItem= async (itemId) =>{
    ...
    const isEnabled = await sudoActivarEnlace(itemId)
    ...
    setBodyList(prevList =>
      prevList.map(item => {
        if (item.id === rowId) {
          ...
          newDataRow[5] = { ...newDataRow[5], text: nuevoEstado };
  ...
  return (
    <Tabla headers={headerTable} body={bodyList}
